...
export function useAdminFilters({ levels = [], specialties = [], data = [] }) {
  ...
  const availableLevels = useMemo(() => {
    ...
    if (filterDegree === "licence") {
      return levels.filter(level => level.code?.toUpperCase().startsWith("L"));
    }
    if (filterDegree === "master") {
      return levels.filter(level => level.code?.toUpperCase().startsWith("M"));
    }
  ...
  const availableSpecialties = useMemo(() => {
    return specialties.filter(specialty => {
      const matchesDegree = filterDegree === "all" || specialty.degree === filterDegree;
      
      // If level is selected, check if specialty is used with this level
      const matchesLevel = filterLevel === "all" || data.some(item =>
        item.program_subjects?.some(ps => 
          ps.specialties?.id === specialty.id && ps.levels?.id === filterLevel
        )
      );
  ...
  const hasActiveFilters = filterDegree !== "all" || 
    filterLevel !== "all" || 
    filterSpecialty !== "all" || 
  ...
  const levelOptions = useMemo(() => [
    ...
    ...availableLevels.map(level => ({ value: level.id, label: level.name }))
  ], [availableLevels]);

  const specialtyOptions = useMemo(() => [
    { value: "all", label: "Toutes les spécialités" },
    ...availableSpecialties.map(s => ({ value: s.id, label: s.name }))
  ...
  const doesExternalFilterPass = useCallback((node) => {
    ...
    if (filterDegree !== "all") {
      ...
      if (item.program_subjects) {
        const matchesDegree = item.program_subjects.some(ps => 
          ps.specialties?.degree === filterDegree
        );
    ...
    if (filterLevel !== "all") {
      if (item.program_subjects) {
        const matchesLevel = item.program_subjects.some(ps => 
    ...
    if (filterSpecialty !== "all") {
      if (item.program_subjects) {
        const matchesSpecialty = item.program_subjects.some(ps => 
          ps.specialties?.id === filterSpecialty
        );
