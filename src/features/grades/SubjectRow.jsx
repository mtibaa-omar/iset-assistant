...
const InputField = ({ label, value, error, onChange }) => (
  <div className="flex items-center gap-2">
    <span className="w-10 text-xs font-medium text-slate-600 dark:text-slate-400 sm:w-11">{label}</span>
    <div className="relative group">
      <input
        ...
        } bg-white dark:bg-slate-900/50 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 transition-all`}
      />
      {error && (
...
export default function SubjectRow({ subject, grades, onGradeChange }) {
  ...
  return (
    <div className="transition-colors bg-white border-b dark:bg-slate-800/50 border-slate-200 dark:border-slate-700/50 last:border-b-0 hover:bg-slate-50 dark:hover:bg-slate-800">
      <div className="flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:py-3">
        ...
        <div className="flex flex-wrap items-center gap-3 sm:flex-nowrap">
          ...
          <div className="flex items-center gap-2 pl-3 ml-3 border-l border-slate-300 dark:border-slate-600">
            <span className="text-xs font-medium text-slate-600 dark:text-slate-400">Moy</span>
            <div
